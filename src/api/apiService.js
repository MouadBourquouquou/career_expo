...
const API_BASE_URL_8081 ='http://localhost:8081/api';
...
export const studentAPI = {
  ...
  getByCompetition: async (competitionId) => {
    const response = await fetch(`${API_BASE_URL_8081}/etudiants/competition/${competitionId}`, {
      headers: { 'Content-Type': 'application/json' }
    });
    if (!response.ok) throw new Error('Failed to fetch students');
    return response.json();
  ...
  delete: async (id) => {
    const response = await fetch(`${API_BASE_URL_8081}/etudiants/${id}`, {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json' }
    });
...
export const competitionAPI = {
  ...
  getByYear: async (year) => {
    const response = await fetch(`${API_BASE_URL_8081}/competitions/annee/${year}`);
    if (!response.ok) throw new Error('Failed to fetch competition');
    return response.json();
  },
